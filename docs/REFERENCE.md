...
Run once from an existing Workbench root:
...
Migration adds `knowledge/providers.json`, updates registry format markers,
canonicalizes task and proposal identifiers to uppercase, and installs the
starter capability catalog into workspaces created before 0.4.0. It never
overwrites a catalog entry you wrote yourself, and it rewrites no knowledge
body, project, or artifact. Running it again is a no-op.
...
awb role|skill|workflow list|show
...
awb profile build|status|complete
awb version
```
